...
void NRF24L01_TX_Mode()
{
  ...
  case 2:
    NRF24_Write_Buf(TX_ADDR, TX_ADDRESS2, TX_ADR_WIDTH);
    NRF24_Write_Buf(RX_ADDR_P0, TX_ADDRESS2, TX_ADR_WIDTH);     
  break;

  case 3:
    NRF24_Write_Buf(TX_ADDR, TX_ADDRESS3, TX_ADR_WIDTH);
    NRF24_Write_Buf(RX_ADDR_P0, TX_ADDRESS3, TX_ADR_WIDTH);     
  break;

  case 4:
    NRF24_Write_Buf(TX_ADDR, TX_ADDRESS4, TX_ADR_WIDTH);
    NRF24_Write_Buf(RX_ADDR_P0, TX_ADDRESS4, TX_ADR_WIDTH);     
  break;

  case 5:
    NRF24_Write_Buf(TX_ADDR, TX_ADDRESS5, TX_ADR_WIDTH);
    NRF24_Write_Buf(RX_ADDR_P0, TX_ADDRESS5, TX_ADR_WIDTH);     
  break;
  
  }
  
  u8Pipe++;
  if(u8Pipe > 5)u8Pipe = 0;
...
void NRF24_ini(void)
{
  ...
  NRF24_WriteReg(EN_RXADDR, 0x01); //для передатчика нужно активировать Pipe0
  NRF24_WriteReg(SETUP_AW, 0x01); // Setup address width=3 bytes
  NRF24_WriteReg(SETUP_RETR, 0xFF); //5F // 1500us, 15 retrans
  ...
  NRF24_WriteReg(RX_PW_P2, TX_PLOAD_WIDTH); //Number of bytes in RX payload in data pipe 1
  NRF24_WriteReg(RX_PW_P3, TX_PLOAD_WIDTH); //Number of bytes in RX payload in data pipe 1
  NRF24_WriteReg(RX_PW_P4, TX_PLOAD_WIDTH); //Number of bytes in RX payload in data pipe 1
  NRF24_WriteReg(RX_PW_P5, TX_PLOAD_WIDTH); //Number of bytes in RX payload in data pipe 1
